...
#     ╚═══╝   ╚═════╝ ╚═╝  ╚═╝   ╚═╝   ╚══════╝╚═╝  ╚═╝
#
# DRONA LABS: VORTEX ENGINE - V8.1 ASCENDED CORE (PRO-DEV MASTER)
...
TOKEN_REGEX = [
    ...
    ('NUMBER',    r'\d+(\.\d+)?'),                               
    ('STRING',    r'".*?"|\'.*?\''),                             
    ('KEYWORD',   r'\b(fn|let|mut|if|else|for|in|while|true|false|print|return|break|continue|assert|save|load|into|request)\b'),
...
class VortexEngine:
    ...
    def evaluate(self, expression_tokens):
        ...
        try:
            ...
            # THE V8.1 STRING COERCION FIX: JavaScript-style Type Conversion!
    ...
    def execute(self, tokens):
        ...
        while i < len(tokens):
            ...
            # --- MEMORY: VARIABLE ALLOCATION ---
                ...
                offset = 1
                if i + 1 < len(tokens) and tokens[i+1][1] == 'mut':
                    offset = 2
            ...
            # --- LANGUAGE: ASSERTIONS ---
            elif kind == 'KEYWORD' and value == 'assert':
                i += 2 # skip 'assert' and '('
                arg_tokens = []
                paren_count = 1
                while i < len(tokens) and paren_count > 0:
                    ...
                    if paren_count > 0: arg_tokens.append(tokens[i])
                    i += 1
                ...
                for tk in arg_tokens:
                    ...
                    if tk[1] == ',' and p_level == 0 and not comma_found:
                        comma_found = True
                        continue
                        
                    if not comma_found: cond_tokens.append(tk)
                    else: msg_tokens.append(tk)
                
                cond_result = self.evaluate(cond_tokens)
                if not cond_result:
                    if msg_tokens:
                        msg_str = self.evaluate(msg_tokens)
                        raise VortexRuntimeError(f"Assertion failed: {msg_str}")
                    else:
                        raise VortexRuntimeError("Assertion failed")
                if i < len(tokens) and tokens[i][1] == ';':
                    i += 1
                continue
            ...
            # --- I/O: WEB FETCHING ---
                ...
                i += 1
                url_tokens = []
                while i < len(tokens) and tokens[i][1] != 'into':
                    url_tokens.append(tokens[i])
                    i += 1
                i += 1 # skip 'into'
                var_name = tokens[i][1]
                i += 1
                ...
                url_str = self.evaluate(url_tokens)
                
                try:
                    req = urllib.request.Request(str(url_str), headers={'User-Agent': 'Vortex-Engine/1.0'})
                    with urllib.request.urlopen(req) as response:
                        ...
                        try:
                            ...
                            parsed_data = data
                        self.stack.allocate(var_name, parsed_data)
                except Exception as e:
                    self.stack.allocate(var_name, f"WEB_ERROR: {str(e)}")
                continue

            # --- I/O: FILE SAVE ---
            elif kind == 'KEYWORD' and value == 'save':
                i += 1
                var_tokens = []
                while i < len(tokens) and tokens[i][1] != 'into':
                ...
                filename_tokens = []
                while i < len(tokens) and tokens[i][1] != ';':
                    filename_tokens.append(tokens[i])
                    i += 1
                if i < len(tokens) and tokens[i][1] == ';':
                    i += 1
                    
                var_data = self.evaluate(var_tokens)
                filename = self.evaluate(filename_tokens)
                safe_filename = str(filename).replace('\\', '/').split('/')[-1]
                filepath = os.path.join(os.getcwd(), safe_filename)
                
                with open(filepath, 'w', encoding='utf-8') as f:
                    if isinstance(var_data, (dict, list)):
                        json.dump(var_data, f, indent=4)
                    else:
                        f.write(str(var_data))
                continue
            ...
            # --- I/O: FILE LOAD ---
                ...
                try:
                    ...
                    raise VortexRuntimeError(f"Failed to load file '{filename}': {str(e)}")
                continue
            ...
            # --- LOGIC: IF / ELSE GATES ---
                ...
                if condition_result:
                    ...
                    if inner.return_value is not None:
                        ...
                        return
                    if inner.control_flow is not None:
                        self.control_flow = inner.control_flow
                        return
            ...
            elif kind == 'KEYWORD' and value == 'break':
                self.control_flow = 'break'
                i += 1
                if i < len(tokens) and tokens[i][1] == ';': i += 1
                return
            elif kind == 'KEYWORD' and value == 'continue':
            ...
            # --- RECURSION: WHILE LOOPS ---
                ...
                while self.evaluate(cond_tokens) and loop_safety < 50000:
                    ...
                    if inner.return_value is not None:
                        self.return_value = inner.return_value
                        return
                    if inner.control_flow == 'break':
                        break
                    elif inner.control_flow == 'continue':
                        pass
                    loop_safety += 1
            ...
            # --- RECURSION: FOR LOOPS ---
                ...
                var_name = tokens[i+1][1]
                i += 3 # skip 'for', var_name, 'in'
                range_tokens = []
                while i < len(tokens) and tokens[i][1] != '{':
                    range_tokens.append(tokens[i])
                    i += 1
                iterable = self.evaluate(range_tokens)
                block_tokens, new_i = self.get_block(tokens, i)
                i = new_i
                ...
                if isinstance(iterable, (list, tuple, str)):
                    for item in iterable:
                        inner = VortexEngine(self.stack, self.functions)
                        inner.stack.allocate(var_name, item)
                        inner.execute(block_tokens)
                        if inner.return_value is not None:
                            self.return_value = inner.return_value
                            return
                    ...
                    raise VortexRuntimeError(f"Cannot iterate over type '{type(iterable).__name__}' in for loop")
                continue
...
def print_header():
    ...
    print("   ╚████╔╝ ╚██████╔╝██║  ██║   ██║   ███████╗██╔╝ ██╗")
    print("    ╚═══╝   ╚═════╝ ╚═╝  ╚═╝   ╚═╝   ╚══════╝╚═╝  ╚═╝")
    print("         DRONA LABS: V8.1 ASCENDED CORE ONLINE")
    print("="*65 + "\n")
