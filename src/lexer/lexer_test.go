...
func TestBasicTokens(t *testing.T) {
	tokens, err := Lex("let x = 42;", "test.vx")
	if err != nil {
		t.Fatal(err)
	}
	if len(tokens) < 5 {
...
func TestKeywords(t *testing.T) {
	tokens, err := Lex("fn if else for while return break continue struct model train", "test.vx")
	if err != nil {
		t.Fatal(err)
	}
...
func TestNumbers(t *testing.T) {
	...
	for _, tt := range tests {
		tokens, err := Lex(tt.input, "test.vx")
		if err != nil {
			t.Fatal(err)
		}
		found := false
...
func TestStrings(t *testing.T) {
	tokens, err := Lex("print \"hello world\";", "test.vx")
	if err != nil {
		t.Fatal(err)
	}
	found := false
...
func TestFloats(t *testing.T) {
	tokens, err := Lex("let pi = 3.14159;", "test.vx")
	if err != nil {
		t.Fatal(err)
	}
	found := false
