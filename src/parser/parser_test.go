...
func TestLetStatement(t *testing.T) {
	prog := parseInput(t, "let x = 42;", "test.vx")
	if len(prog.Stmts) != 1 {
...
func TestLetMut(t *testing.T) {
	prog := parseInput(t, "let mut count = 0;", "test.vx")
...
func TestFunctionDef(t *testing.T) {
	...
	prog := parseInput(t, input, "test.vx")
	if len(prog.Stmts) != 1 {
		t.Fatalf("expected 1 statement, got %d", len(prog.Stmts))
	}
...
func TestNoiseFiltered(t *testing.T) {
	...
	tokens, err := lexer.Lex(input, "test.vx")
	if err != nil {
		t.Fatal(err)
	}
	tokens = lexer.FilterNoise(tokens)
	p := New(tokens, "test.vx")
	prog := p.Parse()
	if len(p.errs) > 0 {
