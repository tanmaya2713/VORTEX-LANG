...
func TestLetInferI32(t *testing.T) {
	c := parseAndCheck(t, "let x = 42;", "test.vx")
	if len(c.errs) > 0 {
...
func TestLetWithTypeAnnotation(t *testing.T) {
	c := parseAndCheck(t, "let x: f64 = 42;", "test.vx")
	if len(c.errs) > 0 {
		t.Fatalf("unexpected errors: %v", c.errs)
	}
...
func TestLetWithTypeMismatch(t *testing.T) {
	c := parseAndCheck(t, "let x: bool = 42;", "test.vx")
	if len(c.errs) == 0 {
...
func TestLetString(t *testing.T) {
	c := parseAndCheck(t, `let name = "vortex";`, "test.vx")
	if len(c.errs) > 0 {
		t.Fatalf("unexpected errors: %v", c.errs)
	}
...
func TestLetBool(t *testing.T) {
	c := parseAndCheck(t, "let yes = true;", "test.vx")
	if len(c.errs) > 0 {
		t.Fatalf("unexpected errors: %v", c.errs)
	}
...
func TestBinaryArithmetic(t *testing.T) {
	c := parseAndCheck(t, "let x = 1 + 2 * 3;", "test.vx")
	if len(c.errs) > 0 {
		t.Fatalf("unexpected errors: %v", c.errs)
	}
	typ, _ := c.globals.lookup("x")
...
func TestBinaryStringConcat(t *testing.T) {
	c := parseAndCheck(t, `let x = "hello " + "world";`, "test.vx")
	if len(c.errs) > 0 {
		t.Fatalf("unexpected errors: %v", c.errs)
	}
	typ, _ := c.globals.lookup("x")
...
func TestBinaryTypeError(t *testing.T) {
	c := parseAndCheck(t, `let x = "hello" + 42;`, "test.vx")
	if len(c.errs) == 0 {
...
func TestComparison(t *testing.T) {
	c := parseAndCheck(t, "let cmp = 1 < 2;", "test.vx")
	if len(c.errs) > 0 {
		t.Fatalf("unexpected errors: %v", c.errs)
	}
...
func TestUnaryNot(t *testing.T) {
	c := parseAndCheck(t, "let ok = !true;", "test.vx")
	if len(c.errs) > 0 {
		t.Fatalf("unexpected errors: %v", c.errs)
	}
...
func TestFunctionDefAndCall(t *testing.T) {
	...
	c := parseAndCheck(t, input, "test.vx")
	if len(c.errs) > 0 {
		t.Fatalf("unexpected errors: %v", c.errs)
	}
}
...
func TestArrayLiteral(t *testing.T) {
	c := parseAndCheck(t, "let arr = [1, 2, 3];", "test.vx")
	if len(c.errs) > 0 {
		t.Fatalf("unexpected errors: %v", c.errs)
	}
...
func TestPrintTypeCheck(t *testing.T) {
	c := parseAndCheck(t, `print "hello";`, "test.vx")
	if len(c.errs) > 0 {
		t.Fatalf("unexpected errors: %v", c.errs)
	}
}
