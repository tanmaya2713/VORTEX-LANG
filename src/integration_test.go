...
func TestClangEndToEnd(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tokens, err := lexer.Lex(tt.source, "test.vx")
			if err != nil {
				t.Fatalf("lex: %v", err)
			}
			tokens = lexer.FilterNoise(tokens)
			p := parser.New(tokens, "test.vx")
			prog := p.Parse()
			if len(p.Errors()) > 0 {
			...
			clangArgs := []string{llFile, filepath.Join(runtimeDir, "io.c"), filepath.Join(runtimeDir, "tensor.c"), "-I" + runtimeDir}
			if runtime.GOOS != "windows" {
				clangArgs = append(clangArgs, "-lm")
			}
			clangArgs = append(clangArgs, "-o", binPath)
			...
			output := strings.ReplaceAll(string(out2), "\r\n", "\n")
